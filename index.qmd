...
As of `r Sys.Date()`, there are currently `r nrow(has_deadline)` packages at risk for removal from CRAN if not fixed by the specified deadlines. (Note that this table does not included information about extra checks, which may be the reason a package has been given a deadline.)
...
(Source code for this site can be found at <https://github.com/hadley/cran-deadlines> and you can see when it was last updated run at <https://github.com/hadley/cran-deadlines/actions/workflows/render.yaml>)
