...
problem_summary <- function(package, version, flavor, status, problems) {
  ...
  paste0(
    glue("<details id='{package}'>"),
    glue("<summary>{package} {version}</summary>"), "\n",
    "\n",
    "Problems: \n\n",
    problems,
    "\n", "\n",
...
function expandDetailsFromHash() {
    const fragment = window.location.hash.substring(1); // Remove the # from the hash
    if (fragment) {
        const detailsElement = document.getElementById(fragment);
...
// Expand details on initial page load
document.addEventListener("DOMContentLoaded", expandDetailsFromHash);

// Expand details whenever the hash changes
